...
public class Ex6_Array {
	...
	public static void main(String[] args) {
		...
		*/
		...
		ar3[0] = ar; //3차원배열의 첫번째 요소에 2차원배열 ar을 저장
		ar3[1] = ar2;
		
		for(int i=0; i<ar3.length; i++) { // 3차원 배열에 저장된 2차원 배열을 반복할 for문
			
			System.out.println("2차원배열 "+ (i+1));
			for(int j=0; j<ar3[i].length; j++) { // 2차원배열에 저장된 1차원배열 반복문
				System.out.print("\r\n\t");//줄바꿈 후 탭을 누른 것
				for(int k=0; k<ar3[i][j].length; k++) { //1차원배열에 저장된 값들 반복처리
