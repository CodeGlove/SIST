...
public class Cart {
     ...
     */
    private static List<ProductVO> list;//장바구니의 저장소
    private long totalPrice;//총액

    public Cart(){//생성자 - useBean정의시 자동 호출됨!
        list = new ArrayList<>();
    }
    //징바구니의 저장소(list)에서 특정 제품(ProductVO)을 검색하는 기능

    public ProductVO searchProduct(String pnum){
        ProductVO vo = null;

        //제품들은 모두 list에 있으므로 list가 비었다는것은
        //사용자가 장바구니에 아무것도 담지 않았다는 뜻이다.
        for (ProductVO pvo : list){//반복문은 널이면 오류지만 0이면 실행안된다.
    ...
    //장바구니의 모든제품을 배열로 반환하는 기능
    public ProductVO[] getList(){
        ProductVO[] ar = null;

        //list가 비었을때는 하지 않아야 한다.
        if(!list.isEmpty()){
    ...
    //p_list는 실제 마트의 진열대와 같다. 이 진열대에 제품을 가져와서 장바구니에 저장
    public void addProduct(ShopBean sb, String pnum){
        //진열대에서 원하는 제품을 가져온다.
        //가져온 제품이 장바구니에 이미 저장되어 있는지? 확인하자 !
        ProductVO vo = searchProduct(pnum);//vo가 null이 아니면 장바구니에 이미 저장한 제품이다.
        ...
        if(vo != null){
            ...
            int q = vo.getQuant();
            vo.setQuant(q+1);
            return;//수량이 증가했으므로 더이상 하지않고 제어권 반환!
        }
        //=========
    ...
    //장바구니에서 특정제품을 검색한 후 수량만 변경하는 기능
