...
<input type="hidden" name="cPath" value="<%=dir%>"/>--%>
      ...
      <p class="btn">
        <a href="javascript:closeUpload()">닫기</a>
      </p>
    </form>
...
<div id="folder_win" title="폴더만들기">
  <form action="makeFolder.jsp" method="post" name="frm3">
    ...
    <p class="btn">
      <a href="javascript:closeFolder()">닫기</a>
    </p>
  </form>
...
<script>
  ...
  function down(fname) {
    //인자로 받은 파일명(fname)을 현재 문서안에 있는 ff라는 이름으로 폼 객체안에
    //이름이 f_name이라는 input type = "hidden"요소의 값(value)로 지정한다.
    ...
    //다운로드가 진행되면서 현재 페이지가 유지될 때 파일명이 지정되어 있으면
    //오류가 발생할 수 있으므로 파일명을 삭제한다.
