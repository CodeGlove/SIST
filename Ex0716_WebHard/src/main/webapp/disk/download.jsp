...
<%
  ...
  //절대경로를 만드는이유: 파일객체를 만들어서 존재여부를 확인하려고
  ...
  if(f.exists()){
    ...
    //읽기한 후 보내야 하므로 InputStream과 OutputStream을 모두 사용해야함
    BufferedInputStream bis = null;
    FileInputStream fis = null;

    BufferedOutputStream bos = null;
    ServletOutputStream sos = null;//********요청자에게 응답으로 스트림을줘야
                                    //다운로드가 된다. response를통해 얻을 수 있는output스트림이
                                    //ServletOutputStream밖에 없다.
     ...
     try{
      ...
      //응답스트림 생성
       ...
       while ((size = bis.read(buf)) != -1){
          //읽은 자원은 buf에 저장되며, 읽은바이트 수는 size가 기억하고 있는 상태다.
