...
<%--
    ...
    그렇게 되면 절대로 request를 통해 파라미터를 받을 수 없다.
    해결책은 무조건 MultipartRequest객체를 통해 받아야 한다.
    그러기 위해서는 라이브러리가 필요함.
...
<%
    ...
    //request를 통해 파라미터들을 받을수 없어서 session에 파일을 저장할 경로(dir)
    //저장하여 넘어왔다.
    ...
    //여기까지만해도 전달되는 첨부파일이 realPath경로에 저장된다.
...
<body onload="movePage()"> <!--바디가끝나면 movePage로 이동  -->
